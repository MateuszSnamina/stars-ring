...
namespace {
    ...
    private:
        ...
        const std::array<ArgAndExpansion, 3> _data; // three values for which f'(x)=0;
    };

    AlphaCos2PlusBetaCos::AlphaCos2PlusBetaCos(double alpha, double beta) :
    _alpha(alpha), _beta(beta),
    _data{
        arg_and_expansion(0.0),
        arg_and_expansion(std::acos(-_beta / 2.0 / _alpha)),
        arg_and_expansion(arma::datum::pi)
    }
    {
        std::cout << "[DEBUG  ] minimizer alpha, beta   : " << _alpha << " " << _beta << "." << std::endl;
        std::cout << "[DEBUG  ] minimizer x, f, f', f'' : " << _data[0].x << " " << _data[0].f << " " << _data[0].fp << " " << _data[0].fpp << "." << std::endl;
        std::cout << "[DEBUG  ] minimizer x, f, f', f'' : " << _data[1].x << " " << _data[1].f << " " << _data[1].fp << " " << _data[1].fpp << "." << std::endl;
        std::cout << "[DEBUG  ] minimizer x, f, f', f'' : " << _data[2].x << " " << _data[2].f << " " << _data[2].fp << " " << _data[2].fpp << "." << std::endl;
    }

    double
    AlphaCos2PlusBetaCos::f(double x) const {
        return _alpha * std::pow(std::cos(x), 2) + _beta * std::cos(x);
    }

    double
    AlphaCos2PlusBetaCos::fp(double x) const {
        return -(2 * _alpha * std::cos(x) + _beta) * std::sin(x);
    }

    double
    AlphaCos2PlusBetaCos::fpp(double x) const {
        return 2 * _alpha * std::pow(std::sin(x), 2)
                -(2 * _alpha * std::cos(x) + _beta) * std::cos(x);
    }

    ArgAndExpansion
    AlphaCos2PlusBetaCos::arg_and_expansion(double x) const {
        return {x, f(x), fp(x), fpp(x)};
    }

    ArgAndExpansion
    AlphaCos2PlusBetaCos::get_minimum() const {
        if (_alpha == 0.0 && _beta == 0.0) {
            std::cerr << "[WARNING] cannnot mimimize f(x) = 0 * cos(x)^2 + 0 * cos(x) function." << std::endl;
        }
        ArgAndExpansion result;
        result = _data[0];
        for (unsigned i = 1; i < _data.size(); i++) {
            if (result.f > _data[i].f)
                result = _data[i];
        }
    ...
    /*
        double calculate_theta_opt_nell(
                ...
                double J, double Ez) {
            const stars_ring_analytical::AnalyticalFormulasBoxAfSpins analytical_formulas_box_af_spins(physical_system, multiplicity);
            const double alpha =
                    J * A * D * physical_system->n_sites() +
                    J * B * D * analytical_formulas_box_af_spins.ground_state_classical_energy();
            const double beta = physical_system->n_sites() * (-Ez) * (-1.0 / 2.0);
        ...
        double calculate_theta_opt_corrected_nell(
            ...
            const double alpha =
                    J * A * D * physical_system->n_sites() +
                    J * B * D * analytical_formulas_box_af_spins.ground_state_energy();
            const double beta = physical_system->n_sites() * (-Ez) * (-1.0 / 2.0);
            AlphaCos2PlusBetaCos fun(alpha, beta);
            return fun.get_minimum().x;
        }
...
namespace stars_ring_analytical {
    ...
    _J(J), _Ez(Ez),
    _theta(theta) {
        std::cout << "[DEBUG  ] [AnalyticalFormulasBoxSu4] A, B, C, D : " << _A << ", " << _B << ", " << _C << ", " << _D << std::endl;
        std::cout << "[DEBUG  ] [AnalyticalFormulasBoxSu4] J, Ez      : " << _J << ", " << _Ez << std::endl;
    }
    ...
    double AnalyticalFormulasBoxJABCDZ::ground_state_classical_energy() const {
        return _J * _A * mean_orbital_operator() * physical_system()->n_sites() +
                J_spin() * _analytical_formulas_box_af_spins.ground_state_classical_energy() -
                _Ez * (-std::cos(_theta) / 2.0) * physical_system()->n_sites();
    }

    double AnalyticalFormulasBoxJABCDZ::ground_state_correlation_energy() const {
        return J_spin() * _analytical_formulas_box_af_spins.ground_state_correlation_energy();
    }

    double AnalyticalFormulasBoxJABCDZ::exc_state_relative_energy(unsigned nk) const {
        return J_spin() * _analytical_formulas_box_af_spins.exc_state_relative_energy(nk);
    }

    double AnalyticalFormulasBoxJABCDZ::theta() const {
        return _theta;
    }
    ...
    double AnalyticalFormulasBoxJABCDZ::theta_opt_nell() const {
        const stars_ring_analytical::AnalyticalFormulasBoxAfSpins analytical_formulas_box_af_spins(physical_system(), _multiplicity);
        const double alpha =
                _J * _A * _D * physical_system()->n_sites() +
                _J * _B * _D * analytical_formulas_box_af_spins.ground_state_classical_energy();
