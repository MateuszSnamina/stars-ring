...
double AnalyticalFormulasBoxAfSpins::ground_state_correlation_energy() const {
  const double S = (_multiplicity - 1) / 2.0;
  return S * _analytical_formulas_box_af_oscylators
                 .ground_state_correlation_energy();
}

double AnalyticalFormulasBoxAfSpins::exc_state_relative_energy(
    unsigned nk) const {
  const double S = (_multiplicity - 1) / 2.0;
  return S *
         _analytical_formulas_box_af_oscylators.exc_state_relative_energy(nk);
}
}  // namespace stars_ring_analytical