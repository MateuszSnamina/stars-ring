...
BasisBox BasisBox::Builder::build(
  ...
  std::shared_ptr<CoupledElementsGenerator<LocalizedElement>>
      coupled_localized_elements_generator =
          std::make_shared<CoupledLocalizedElementsGenerator>(
              coupled_raw_states_generator);
  const LocalizedBasis localized_basis = build_localized_basis(
      physical_system, init_states, coupled_localized_elements_generator,
      perturbation_level);
  ...
  std::shared_ptr<CoupledElementsGenerator<TranslationUniqueElement>>
      coupled_translation_unique_elements_generator =
          std::make_shared<CoupledTranslationUniqueElementsGenerator>(
              coupled_raw_states_generator);
  const TranslationUniqueBasis translation_unique_basis =
      build_translation_unique_basis(
          physical_system, init_states,
          coupled_translation_unique_elements_generator, perturbation_level);
  // Build ksubspace_basis.
  const std::vector<KsubspaceBasis> ksubspace_basis =
      transform_translation_unique_basis_to_ksubspace_basis(
          physical_system, translation_unique_basis);
  // Return all the basis:
  return BasisBox(physical_system, localized_basis, translation_unique_basis,
                  ksubspace_basis);
}
