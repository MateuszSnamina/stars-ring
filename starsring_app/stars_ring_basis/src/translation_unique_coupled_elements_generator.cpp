...
CoupledTranslationUniqueElementsGenerator::
    CoupledTranslationUniqueElementsGenerator(
...
CoupledTranslationUniqueElementsGenerator::generate(
    std::shared_ptr<TranslationUniqueElement> element) const {
  assert(element);
  assert(element->physical_system());
  assert(_coupled_raw_states_generator);
  const std::vector<RawState> raw_coupled_states =
      _coupled_raw_states_generator->generate(
          element->raw_representative_state());
  ...
  for (const RawState& raw_coupled_state : raw_coupled_states) {
    result.push_back(stars_ring_basis::make_translation_unique_element(
        element->physical_system(), raw_coupled_state,
