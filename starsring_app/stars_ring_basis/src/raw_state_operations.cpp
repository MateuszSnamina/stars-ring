    ...
    namespace operations {
        ...
        double determine_coupling(
            ...
            for (unsigned i = 0, j = 1; i < size; i++, j = (j + 1) % size) {
                {
                    if (bra == ket) {
                        result -= spin_realm.S_z(bra[i], ket[i]) * spin_realm.S_z(bra[j], ket[j]);
                ...
                }
                {
                    ...
                    if (bra == ket_pp) {
                        result += spin_realm.S_p(bra[i], ket[i]) * spin_realm.S_p(bra[j], ket[j]) / 2.0;
                ...
                }
                {
                    ...
                    if (bra == ket_mm) {
                        result += spin_realm.S_m(bra[i], ket[i]) * spin_realm.S_m(bra[j], ket[j]) / 2.0;
                        break; //only for performance optimization.
                    }
                }
            }
