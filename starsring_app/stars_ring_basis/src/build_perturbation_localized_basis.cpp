...
namespace stars_ring_basis {
    ...
    LocalizedBasis build_localized_basis(
        ...
        std::cout << "[INFO   ] [PROGRESS] Program is about to construct localized-basis." << std::endl;
        ...
        for (const RawState& init_state : init_states) {
            for (unsigned i = 0; i < physical_system->n_cells(); i++) {
                ...
                localized_basis.add_element(make_localized_element(physical_system, temp, 0));
            }
        }
        PerturbationBasisMaker<LocalizedElement> perturbation_basis_generator(coupled_localized_elements_generator);
