...
namespace stars_ring_basis {
    ...
    public:
        ...
        const std::shared_ptr<TranslationUniqueElement> translation_unique_element() const;
        unsigned nk() const;
        double norm() const;
        // Factory:
    ...
    std::ostream& operator<<(
            std::ostream& s,
            const KsubspaceElement& el);

    // Inline functions implementations:

    inline
    const std::shared_ptr<TranslationUniqueElement>
    ...
    inline
    unsigned
    KsubspaceElement::nk() const {
        return _nk;
    }

    inline
    typename KsubspaceElement::KeyT
    KsubspaceElement::key() const {
        return translation_unique_element()->key();
    }

    inline
    double
    KsubspaceElement::norm() const {
        return translation_unique_element()->norm(_nk);
    }

    inline
    std::unique_ptr<KsubspaceElement>
    make_ksubspace_element(
            std::shared_ptr<TranslationUniqueElement> translation_unique_element,
            unsigned nk) {
        std::unique_ptr<KsubspaceElement> result(new KsubspaceElement(translation_unique_element, nk));
        return result;
    }
