...
std::vector<std::vector<
    unsigned>>  // the first idx is a TranslationUniqueElement index.
construct_translation_unique_basis_to_localized_basis_linker(
    const TranslationUniqueBasis& translation_unique_basis,
    const LocalizedBasis& localized_basis);

std::vector<std::vector<LinearCombination<
    std::complex<double>,
    unsigned>>>  // the first idx corresponds to nk, the second is an
                 // KsubspaceElement index.
...
//    unsigned>>> construct_ksubspace_basis_to_localized_basis_linker(
