...
class CoupledElementsGenerator {
 public:
  virtual std::vector<std::shared_ptr<Element>> generate(
      std::shared_ptr<Element>) const = 0;
...
class PerturbationBasisMaker {
 public:
  PerturbationBasisMaker(std::shared_ptr<CoupledElementsGenerator<Element>>
                             coupled_elements_generator);
  // strategy:
  std::shared_ptr<CoupledElementsGenerator<Element>>
  coupled_elements_generator() const;
...
template <typename Element>
std::shared_ptr<CoupledElementsGenerator<Element>>
PerturbationBasisMaker<Element>::coupled_elements_generator() const {
  return _coupled_elements_generator;
}

template <typename Element>
void PerturbationBasisMaker<Element>::coupled_elements_generator(
    std::shared_ptr<CoupledElementsGenerator<Element>>
        coupled_elements_generator) {
  _coupled_elements_generator = coupled_elements_generator;
}
...
template <typename Element>
void PerturbationBasisMaker<Element>::populate(Basis<Element>& basis,
                                               unsigned max_order) {
  ...
  for (unsigned order = 0; order < max_order; order++) {
    ...
    for (unsigned idx = old_size - last_chunk_size; idx < old_size; idx++) {
      ...
      const std::vector<std::shared_ptr<Element>> new_elements =
          _coupled_elements_generator->generate(el);
