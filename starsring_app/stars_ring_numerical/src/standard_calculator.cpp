...
namespace stars_ring_numerical {
    ...
    std::shared_ptr<HamiltonianCreator> StandardCalculator::hamiltonian_creator() const {
        return _hamiltonian_creator;
    }

    void StandardCalculator::hamiltonian_creator(std::shared_ptr<HamiltonianCreator> hamiltonian_creator) {
        _hamiltonian_creator = hamiltonian_creator;
    }
    ...
    void StandardCalculator::do_calculations_sparse_create_hamiltonian() {
        ...
        timer.tic();
        std::cout << "[INFO   ] [PROGRESS] [SPARSE] Program is about to build the hamiltonian." << std::endl;
        _hamiltonian_creator->creat_hamiltonian(_hamiltonian_sparse, _basis_box.localized_basis());
    ...
    void StandardCalculator::do_calculations_sparse_diagonalize() {
        std::cout << "[INFO   ] [PROGRESS] [SPARSE] Program is about to diagonalize the hamiltonian." << std::endl;
        arma::wall_clock timer;
        timer.tic();
        if (!arma::eigs_sym(_energies, _states, _hamiltonian_sparse, _n_calculated_states, "sa")) {
            std::cerr << "[ERROR  ] Armadillo failed to diagonalize the hamiltonian!" << std::endl;
            std::cerr << "[ERROR  ] Program termination (with exit code 20)." << std::endl;
            exit(20);
        }
        const double calculation_time = timer.toc();
        std::cout << "[INFO   ] [PROGRESS] [SPARSE] Program has diagonalized the hamiltonian"
                << " (in " << calculation_time << "s)." << std::endl;
    }

    void StandardCalculator::do_calculations_sparse(unsigned requested_n_states_to_calculate) {
        assert(_basis_box.localized_basis().physical_system());
        assert(_basis_box.localized_basis().size() > 1);
        do_calculations_sparse_create_hamiltonian();
        _n_calculated_states = std::min(requested_n_states_to_calculate, _basis_box.localized_basis().size() - 1);
    ...
    void StandardCalculator::do_calculations_dense_create_af_hamiltonian() {
        ...
        arma::wall_clock timer;
        timer.tic();
        std::cout << "[INFO   ] [PROGRESS] [DENSE] Program is about to build the hamiltonian." << std::endl;
        _hamiltonian_creator->creat_hamiltonian(_hamiltonian_dense, _basis_box.localized_basis());
        const double calculation_time = timer.toc();
    ...
    void StandardCalculator::do_calculations_dense_diagonalize() {
        std::cout << "[INFO   ] [PROGRESS] [DENSE] Program is about to diagonalize the hamiltonian." << std::endl;
        arma::wall_clock timer;
        timer.tic();
        ...
        const double calculation_time = timer.toc();
        std::cout << "[INFO   ] [PROGRESS] [DENSE] Program has diagonalized the hamiltonian"
                << " (in " << calculation_time << "s)." << std::endl;
    }
    ...
    const arma::sp_mat &
    StandardCalculator::hamiltonian_sparse() const {
        return _hamiltonian_sparse;
    }

    const arma::mat &
    StandardCalculator::hamiltonian_dense() const {
        return _hamiltonian_dense;
    }

    unsigned
    StandardCalculator::n_calculated_states() const {
        return _n_calculated_states;
    }

    const arma::vec &
    StandardCalculator::energies() const {
        return _energies;
    }

    const arma::mat &
    StandardCalculator::states() const {
        return _states;
    }
    ...
    double
    StandardCalculator::energies(unsigned state_idx) const {
        assert(state_idx < _energies.n_elem);
        return _energies(state_idx);
    }

    const arma::vec
    StandardCalculator::states(unsigned state_idx) const {
        assert(state_idx < _states.n_cols);
        return _states.col(state_idx);
    }

    std::tuple<double, const arma::vec&>
    StandardCalculator::energies_states(unsigned state_idx)const {
        assert(state_idx < _states.n_cols);
        assert(state_idx < _energies.n_elem);
