...
SpinHamiltonianCreator::SpinHamiltonianCreator(
    maths_in_physic::SpinRealm spin_realm)
    : _spin_realm(spin_realm) {}

void SpinHamiltonianCreator::creat_hamiltonian(
  ...
  for (unsigned ket_idx = 0; ket_idx < n_elems; ++ket_idx) {
    ...
    for (unsigned bra_idx = 0; bra_idx < n_elems; ++bra_idx) {
      ...
      H(bra_idx, ket_idx) = stars_ring_basis::operations::determine_coupling(
          bra->raw_state(), ket->raw_state(), _spin_realm);
    }
  }
}
  ...
  for (unsigned ket_idx = 0; ket_idx < n_elems; ++ket_idx) {
        ...
        basis.vec_index()[ket_idx];
    {
      ...
      for (unsigned i = 0, j = 1; i < n_sites; i++, j = (j + 1) % n_sites) {
        diag_energy -= _spin_realm.S_z_diag(ket->raw_state()[i]) *
                       _spin_realm.S_z_diag(ket->raw_state()[j]);
      }
    ...
    }
    {
      for (unsigned i = 0, j = 1; i < n_sites; i++, j = (j + 1) % n_sites) {
        ...
        H(*bra_idx, ket_idx) = _spin_realm.S_pm_diag(ket->raw_state()[i]) *
                               _spin_realm.S_pm_diag(ket->raw_state()[j]) / 2.0;
      }
    }
    {
      for (unsigned i = 0, j = 1; i < n_sites; i++, j = (j + 1) % n_sites) {
        ...
        if (!bra_idx)
          continue;  // nie wszedl do bazy, bo baza nie musi byc zupelna!
        H(*bra_idx, ket_idx) = _spin_realm.S_pm_diag(ket->raw_state()[i] - 1) *
                               _spin_realm.S_pm_diag(ket->raw_state()[j] - 1) /
                               2.0;
      }
    }
  }
