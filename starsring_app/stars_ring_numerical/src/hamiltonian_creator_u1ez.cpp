...
namespace stars_ring_numerical {
    ...
    void U1EzHamiltonianCreator::creat_hamiltonian(arma::mat & H, const
stars_ring_basis::LocalizedBasis& basis) const { throw std::logic_error("not
implementd!");
    }

    void U1EzHamiltonianCreator::creat_hamiltonian(arma::sp_mat & H, const
stars_ring_basis::LocalizedBasis& basis) const {
...
basis.vec_index()[ket_idx]; const stars_ring_basis::RawState ket_spin =
ket->raw_state(); const RawPhis ket_phis =
_phis_establisher->establish(ket_spin);
            {
                double diag_energy = 0.0;
                assert(false);
                for (unsigned i = 0, j = 1; i < n_sites; i++, j = (j + 1) %
n_sites) { double spin_op = 3.0 / 4.0 - spin_realm.S_z_diag(ket_spin[i]) *
spin_realm.S_z_diag(ket_spin[j]); double orbit_op =
half_P_zeta_c_P_xi_c(ket_phis, i, j, ket_phis); diag_energy += -2 * _E_U1 *
spin_op * orbit_op;
                }
            ...
            {
                for (unsigned i = 0, j = 1; i < n_sites; i++, j = (j + 1) %
n_sites) { auto temp = stars_ring_basis::operations::crcr(i, j,
ket->raw_state(), spin_realm.multiplicity); if (!temp) continue; const auto
bra_idx = basis.find_element_and_get_its_ra_index(*temp); if (!bra_idx)
continue; // nie wszedl do bazy, bo baza nie musi byc zupelna!
                    std::shared_ptr<stars_ring_basis::LocalizedElement> bra =
basis.vec_index()[*bra_idx]; const stars_ring_basis::RawState bra_spin =
bra->raw_state(); const RawPhis bra_phis =
_phis_establisher->establish(bra_spin); double spin_op = 3.0 / 4.0 -
spin_realm.S_pm_diag(ket_spin[i]) * spin_realm.S_pm_diag(ket_spin[j]) / 2.0;
                    double orbit_op = half_P_zeta_c_P_xi_c(bra_phis, i, j,
ket_phis); H(*bra_idx, ket_idx) = -2 * _E_U1 * spin_op * orbit_op;
                }
            }
            {
                for (unsigned i = 0, j = 1; i < n_sites; i++, j = (j + 1) %
n_sites) { assert(false);

                    auto temp = stars_ring_basis::operations::anan(i, j,
ket->raw_state()); if (!temp) continue; const auto bra_idx =
basis.find_element_and_get_its_ra_index(*temp); if (!bra_idx) continue; // nie
wszedl do bazy, bo baza nie musi byc zupelna! H(*bra_idx, ket_idx) =
spin_realm.S_pm_diag(ket_spin[i] - 1) * spin_realm.S_pm_diag(ket_spin[j] - 1)
/ 2.0;
                }
            }
        }
