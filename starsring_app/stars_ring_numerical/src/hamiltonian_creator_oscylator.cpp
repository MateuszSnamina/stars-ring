...
OscylatorHamiltonianCreator::OscylatorHamiltonianCreator(
    maths_in_physic::OscilatorRealm oscilator_realm)
    : oscilator_realm(oscilator_realm) {}

void OscylatorHamiltonianCreator::creat_hamiltonian(
    ...
    }
    {
      for (unsigned i = 0, j = 1; i < n_sites; i++, j = (j + 1) % n_sites) {
        auto temp = stars_ring_basis::operations::crcr(
            i, j, ket->raw_state(), oscilator_realm.n_max_stars);
        if (!temp) continue;
        const auto bra_idx = basis.find_element_and_get_its_ra_index(*temp);
        if (!bra_idx)
          continue;  // nie wszedl do bazy, bo baza nie musi byc zupelna!
        H(*bra_idx, ket_idx) = oscilator_realm.cr_an_diag(ket->raw_state()[i]) *
                               oscilator_realm.cr_an_diag(ket->raw_state()[j]);
        // debug_licznik++; //debug!!!
      }
    }
    {
      for (unsigned i = 0, j = 1; i < n_sites; i++, j = (j + 1) % n_sites) {
        ...
        if (!bra_idx)
          continue;  // nie wszedl do bazy, bo baza nie musi byc zupelna!
        H(*bra_idx, ket_idx) =
            oscilator_realm.cr_an_diag(ket->raw_state()[i] - 1) *
            oscilator_realm.cr_an_diag(ket->raw_state()[j] - 1);
        // debug_licznik++; //debug!!!
      }
    }
  }
  // std::cout << "debug_licznik in
  // OscylatorHamiltonianCreator::creat_hamiltonian: " << debug_licznik <<
  // std::endl; //debug
}
