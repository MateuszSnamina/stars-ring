...
namespace stars_ring_numerical {
    ...
    void JABCDZHamiltonianCreator::creat_hamiltonian(arma::mat & H, const stars_ring_basis::LocalizedBasis& basis) const {

        throw std::logic_error("not implementd!");
    }

    void JABCDZHamiltonianCreator::creat_hamiltonian(arma::sp_mat & H, const stars_ring_basis::LocalizedBasis& basis) const {
        ...
        for (unsigned ket_idx = 0; ket_idx < n_elems; ++ket_idx) {
            ...
            const RawPhis ket_phis = _phis_establisher->establish(ket_spin);
            {
                ...
                for (unsigned i = 0, j = 1; i < n_sites; i++, j = (j + 1) % n_sites) {
                    const double spin_op = _A - _B * _spin_realm.S_z_diag(ket_spin[i]) * _spin_realm.S_z_diag(ket_spin[j]);
                    const double orbit_op = psi1_orbitij_psi2(ket_phis, i, j, ket_phis, _C, _D);
            ...
            }
            {
                for (unsigned i = 0, j = 1; i < n_sites; i++, j = (j + 1) % n_sites) {
                    ...
                    const double spin_op = _B * _spin_realm.S_pm_diag(ket_spin[i]) * _spin_realm.S_pm_diag(ket_spin[j]) / 2.0;
                    const double orbit_op = psi1_orbitij_psi2(bra_phis, i, j, ket_phis, _C, _D);
            ...
            }
            {
                for (unsigned i = 0, j = 1; i < n_sites; i++, j = (j + 1) % n_sites) {
                    ...
                    const stars_ring_basis::RawState bra_spin = bra->raw_state();
                    const RawPhis bra_phis = _phis_establisher->establish(bra_spin);
                    const double spin_op = _B * _spin_realm.S_pm_diag(ket_spin[i] - 1) * _spin_realm.S_pm_diag(ket_spin[j] - 1) / 2.0;
                    const double orbit_op = psi1_orbitij_psi2(bra_phis, i, j, ket_phis, _C, _D);
                    H(*bra_idx, ket_idx) = _J * spin_op * orbit_op;
                }
            }
