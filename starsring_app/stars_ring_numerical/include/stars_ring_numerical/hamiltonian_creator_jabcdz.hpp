 ...
 * 
 * Operatory orbitalne:
 * tau_c = ( |xi_c><xi_c| - |zeta_c><zeta_c| ) / 2.0   // Konwencja ,,po staremu'' [przed zamiana tau := - tau]
 * np. <zeta|tau_c|zeta> =     <0|tau_c|0> = -1
 * np. <xi|tau_c|xi>     = <180|tau_c|180> = +1
 * 
 * Uwaga: dla C = 0.25, D = 1.0
 *        operator orbitalny to nie jest ani P^{xi,xi}, ani P^{zeta,xi}, ani P^{zeta,zeta},
 *        operaotr orbitalny P^{zeta,xi} to jest (1/4 - \hat tau^c_i * \hat tau^c_j).
 * 
 * Uawga: Dla S=1/2:
 ...
 * H = J * (A - B * \hat K^z_i * \hat K^z_j + 1/2 * B * \hat K^+_i * \hat K^+_j + B * 1/2 * \hat K^-_i * \hat K^-_j) *
 *         (C + D * tau^c_i * tau^c_j)
 *     - Ez * tau^c_i
 * gdzie:
