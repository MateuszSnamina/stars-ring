...
namespace stars_ring_basis {
    ...
    TranslationUniqueBasis build_translation_unique_basis(
        ...
        std::cout << "[INFO   ] [PROGRESS] Program is about to construct translation-unique-basis." << std::endl;
        ...
        for (const RawState& init_state : init_states) {
            translation_unique_basis.add_element(make_translation_unique_element(physical_system, init_state, 0));
        }
        PerturbationBasisMaker<TranslationUniqueElement> perturbation_basis_generator(coupled_translation_unique_elements_generator);
        perturbation_basis_generator.populate(translation_unique_basis, perturbation_level);
        // -------------------------------
        const double calculations_time = timer.toc();
        std::cout << "[INFO   ] [PROGRESS] Program has constructed translation-unique-basis"
