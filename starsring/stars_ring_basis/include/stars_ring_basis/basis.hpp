...
namespace stars_ring_basis {
    ...
    private:
        ...
        VecMapT _vecmap;
    public:
        explicit Basis(std::shared_ptr<stars_ring_core::PhysicalSystem> physical_system);
        ...
        boost::optional<unsigned> find_element_and_get_its_ra_index(typename VecMapT::KeyT v) const;
        unsigned size() const;
    };
    ...
    template <typename Element>
    Basis<Element>::Basis(std::shared_ptr<stars_ring_core::PhysicalSystem> physical_system) :
    ...
    template <typename Element>
    void
    Basis<Element>::add_element(typename Basis<Element>::VecMapT::ElementPtrT c) {
        assert(c);
    ...
    template <typename Element>
    const typename Basis<Element>::VecMapT::VecIndex&
    Basis<Element>::vec_index() const {
        return _vecmap.vec_index();
    }

    template <typename Element>
    const typename Basis<Element>::VecMapT::MapIndex&
    Basis<Element>::map_index() const {
        return _vecmap.map_index();
    }

    template <typename Element>
    boost::optional<unsigned>
    Basis<Element>::find_element_and_get_its_ra_index(typename VecMapT::KeyT v) const {
