...
#define PRINT_ANGLE(phi) std::cout << #phi << " = " << (phi) << " rad =  " << (phi) * ( 180 / arma::datum::pi ) << " st" << std::endl;
...
public:
    ValuePut(std::string title, unsigned title_width,
    ...
    ValuePut(PrintStyle print_style, std::string title, unsigned title_width,
             ...
             std::string unit = std::string());
    template<typename U>
    friend std::ostream & operator<<(std::ostream & stream, const ValuePut<U> & ternary_put);
};
...
private:
    ...
    const std::string m_unit;
public:
    TernaryPut(std::string title, unsigned title_width,
               double x, double y, double z, unsigned value_width = 0, unsigned value_precision = 6,
               std::string unit = std::string());
    TernaryPut(PrintStyle print_style, std::string title, unsigned title_width,
               double x, double y, double z, unsigned value_width = 0, unsigned value_precision = 6,
               std::string unit = std::string());
    TernaryPut(std::string title, unsigned title_width,
               arma::vec3 v, unsigned value_width = 0, unsigned value_precision = 6,
               std::string unit = std::string());
    TernaryPut(PrintStyle print_style, std::string title, unsigned title_width,
               arma::vec3 v, unsigned value_width = 0, unsigned value_precision = 6,
               std::string unit = std::string());
    friend std::ostream & operator<<(std::ostream & stream, const TernaryPut & ternary_put);
};
