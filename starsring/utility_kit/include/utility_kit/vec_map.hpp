...
namespace utility {
     ...
     * (2) Element must define the Element::KeyT Element::key() const member function.
     * The VecMap containers uses above member function as a key for the map.
     *
    ...
    private:
        ...
        typedef boost::multi_index::const_mem_fun<Element, KeyT, &Element::key> KayExtractorDef;
        // Container type definition -- index typedefs:
        typedef boost::multi_index::random_access<VecTagDef> VecIndexDef;
        typedef boost::multi_index::ordered_unique<MapTagDef, KayExtractorDef> MapIndexDef;
        // Container type definition -- final container typedef:
        typedef boost::multi_index::multi_index_container<ElementPtrT, boost::multi_index::indexed_by<VecIndexDef, MapIndexDef>> Container;
        // The container:
        Container container;
    public:
        // Container type definition -- index typedefs:
    ...
    VecMap<Element>::find_element_and_get_its_ra_index(KeyT v) const {
        auto search_iter = map_index().find(v);
        if (search_iter == map_index().end())
