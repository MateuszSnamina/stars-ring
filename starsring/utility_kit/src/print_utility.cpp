...
namespace utility {
    ...
    AbstractPut::AbstractPut(PrintStyle print_style) :
    ...
    template<typename T>
    ValuePut<T>::ValuePut(std::string title, unsigned title_width,
            T value, unsigned value_width, unsigned value_precision,
            std::string unit) :
    ...
    template<typename T>
    ValuePut<T>::ValuePut(PrintStyle print_style, std::string title, unsigned title_width,
            T value, unsigned value_width, unsigned value_precision,
            std::string unit) :
    ...
    std::ostream & operator<<(std::ostream & stream, const ValuePut<T> & value_put) {
        ...
        if (std::is_arithmetic<T>::value) stream << std::fixed << std::boolalpha << std::setprecision(value_put.m_value_precision) << std::showpos;
        stream << std::left
                << std::setw(value_put.m_title_width) << value_put.m_title << " : ";
        ...
        stream.flags(f);
        return stream;
    } // end of std::ostream & operator<<(std::ostream & stream, const ValuePut<T> & value_put)
    ...
    template std::ostream & operator<<(std::ostream &, const ValuePut<std::string> &);

    //#############################################################################
    //######################    TernaryPut               ##########################
    //#############################################################################

    TernaryPut::TernaryPut(std::string title, unsigned title_width,
            double x, double y, double z, unsigned value_width, unsigned value_precision,
            std::string unit) :
    AbstractPut(PrintStyle::plain),
    m_title(title),
    m_title_width(title_width),
    ...
    TernaryPut::TernaryPut(PrintStyle print_style, std::string title, unsigned title_width,
            double x, double y, double z, unsigned value_width, unsigned value_precision,
            std::string unit) :
    AbstractPut(print_style),
    m_title(title),
    ...
    TernaryPut::TernaryPut(std::string title, unsigned title_width,
            arma::vec3 v, unsigned value_width, unsigned value_precision,
            std::string unit) :
    AbstractPut(PrintStyle::plain),
    m_title(title),
    m_title_width(title_width),
    ...
    std::ostream & operator<<(std::ostream & stream, const TernaryPut & ternary_put) {
        ...
        stream << std::fixed << std::setprecision(ternary_put.m_value_precision) << std::showpos;
        stream << std::left
                << std::setw(ternary_put.m_title_width) << ternary_put.m_title << " : "
                << std::right
        ...
        stream.flags(f);
        return stream;
    } // end of std::ostream & operator<<(std::ostream & stream, const TernaryPut & ternary_put)
