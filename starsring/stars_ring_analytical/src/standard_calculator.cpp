...
namespace stars_ring_analytical {
    ...
    StandardCalculator::StandardCalculator(std::shared_ptr<AnalyticalFormulasBox> formulas_box) :
    _formulas_box(formulas_box) {
    }

    void StandardCalculator::calculate() {
        _ground_state_classical_energy = _formulas_box->ground_state_classical_energy();
        _ground_state_correlation_energy = _formulas_box->ground_state_correlation_energy();
        _ground_state_energy = _formulas_box->ground_state_energy();
        const unsigned max_tantamount_kn = _formulas_box->physical_system()->n_cells() / 2 + 1;
        _exc_state_relative_energy.resize(max_tantamount_kn);
        for (unsigned nk = 0; nk < max_tantamount_kn; ++nk)
            _exc_state_relative_energy[nk] = _formulas_box->exc_state_relative_energy(nk);
    }

    void StandardCalculator::formulas_box(std::shared_ptr<AnalyticalFormulasBox> formulas_box) {
        _formulas_box = formulas_box;
    }

    std::shared_ptr<AnalyticalFormulasBox> StandardCalculator::formulas_box() const {
